...
Typische Anwendungen des Runs-Test sind die Analyse von Zeitreihen, z.B. in der Qualitätssicherung (u.a. zur Zufälligkeit der Verteilung von Ereignissen wie z.B. Defekten), aber auch zum Test von Pseudozufallszahlengeneratoren (PRNGs).

Der Run-Test kann angewendet werden, um Stationarität bzw. Nicht-Korrelation in einer Zeitreihe oder anderen Sequenz zu überprüfen, **insbesondere auch dann, wenn die Verteilung des Merkmals unbekannt ist**. Die [Nullhypothese](https://de.wikipedia.org/wiki/Hypothese_(Statistik)#Nullhypothese) ist hier, dass aufeinanderfolgende Werte unkorreliert sind.

::: {.callout-tip}
## Grundgesamtheit mit mehr als zwei Ausprägungen des Merkmals
Liegt eine endliche Folge reeller Zahlen $(x_{i})$ eines metrischen Merkmals vor, wird die Folge zunächst dichotomisiert: Man bestimmt zunächst den Median $\tilde{x}$ der Folge. Werte $x_{i}< \tilde{x}$ werden dann als Kugeln/Ereignesse der ersten Sorte, Werte $x_{i}> \tilde{x}$ als Kugeln/Ereignisse der zweiten Sorte interpretiert. Die entstandene dichotome Folge kann dann wieder auf die bereits beschriebene Weise auf Zufälligkeit getestet werden.
:::
...
Da aktuell keine im Experiment erhobenen Daten vorliegen bleibt dies aber bis zur Durchführung einer Verkehrszählung nur eine Theorie, der der geneigte Leser folgen kann oder auch nicht.
...
<!-- -------------------------------------------------------------------------
                       ...
                       Runs-Test: Fairer sechsseitiger Würfel - 100 Würfe

-------------------------------------------------------------------------- -->
<!-- ::: {.content-hidden unless-meta="exercises.solutions"} -->
<!-- ::: {.callout-note collapse=true} -->
::: {.callout-tip} 
## Lösung: Runs-Test *"Fairer sechsseitiger Würfel"*


**Ausgangssituation**

Mit einem **fairen Würfel (6 Seiten)** werden **100 Würfe** durchgeführt. Mittels eines **Runs-Tests** (Test auf Unabhängigkeit/Zufälligkeit) soll geprüft werden ob die Abfolge der Ergebnisse zufällig erscheint.

Der **Runs-Test** untersucht, ob eine Folge von Beobachtungen **zufällig** ist oder ob es **Abhängigkeiten oder Muster** gibt. Ein Run ist hierbei eine **Serie identischer oder gleichartig klassifizierter Werte**, die von einem andersartigen Wert unterbrochen wird. Der Runs-Test basiert auf der Wahrscheinlichkeit, dass zwei benachbarte Werte gleich oder unterschiedlich sind.

Da beim Würfeln Zahlen von 1–6 auftreten müssen die Ergebnisse der Würfe zunächst dichotomisiert werden. Hierzu gibt es mehrere mögliche Vorgehensweisen. Dem geneigten Leser sein hier zwei Varianten angeboten:

+ **Variante A:** Aufteilung der Ergebnisse in **zwei Gruppen** (z.B. *"hoch"* = 4–6, *"niedrig"* = 1–3) und Zählung der Runs von *"hoch"* und "*niedrig"*.
+ **Variante B:** Aufteilen in Runs **gleicher Zahl** (z.B. *"2,2,2"* entspricht einem Run von drei Zweiern).

<!--
* **Variante C:** Du testest über oder unter dem Median (häufig bei kontinuierlichen Daten).
-->

**Erwartete Ergebnisse bei einem fairen Würfel (Variante A: *"hoch, niedrig"*)**

Die erwartete Anzahl der Runs $E(R)$, Streuung und Varianz bei einem fairen Würfel und 100 Würfen ($n_1 ≈ n_2 ≈ 50$) sind gegeben durch:

$$
E(R) = \frac{2 n_1 n_2}{n_1 + n_2} + 1 = 51
$$

$$
Var(R) = \frac{2 n_1 n_2 \cdot (2 \cdot n_1 n_2 - n_1 - n_2)}{(n_1 + n_2)^2 \cdot (n_1 + n_2 - 1)} \approx 24.75
$$

$$
\sigma_R = \sqrt{Var(R)} \approx 4.97
$$

Unter Zufälligkeit (gewähltes Vertrauensintervall: $2 \cdot \sigma_R$) liegt die erwartete Anzahl der Runs **im Bereich von ca. 51 ± 10**, also zwischen etwa **41 und 61**. Liegt das vom geneigten Leser ermittelte Ergebnis in diesem Bereich kann der Würfel als **mit hoher Wahrscheinlichkeit fair** bewertet werden.


**Erwartete Ergebnisse bei einem fairen Würfel (Variante B: *"Runs gleicher Zahl"*)**

Die Wahrscheinlichkeit, dass zwei gleiche Zahlen hintereinander kommen, ist $(1/6)$. Zu erwarten sind daher zumeist nur sehr kurze Runs (meist nur Länge 1); die erwartete Anzahl der Runs kann in guter Näherung berechnet werden ≈ Gesamtzahl der Würfe minus die Anzahl der aufeinanderfolgenden Wiederholungen $(99 \times \frac{1}{6} ≈ 16.5)$.

Bei 100 Würfen sind somit als Mittelwert **83–84 Runs gleicher Zahl** zu erwarten. Die zu erwartende Streuung resultiert aus der Normalverteilung der Runs um diesen Mittelwert.


**Interpretation der Ergebnisse**


| Variante                   | Erwartete Anzahl Runs | Typischer Bereich | Interpretation |
| -------------------------- | --------------------- | ----------------- | -------------- |
| Hoch–Niedrig (1–3 vs. 4–6) | ~51                   | 41–61             | Zufällig       |
| Gleiche Zahl               | ~83–84                | ~75–92            | Zufällig       |

: Erwarte Anzahl von Runs bei einem fairen Würfel (100 Würfe) {#tbl-fairerwuerfel .striped .hover .bordered}

Ergebnisse außerhalb dieser Bereiche weisen auf **Nicht-Zufälligkeit** (z.B. nicht-fairer Würfel oder Zahlenfolge ist nicht durch Würfeln entstanden) hin.
:::